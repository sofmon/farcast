...
- **Scopes are minted at deploy.** `farcast run` mints `app/<ns>/<app>/` per application and says so, carrying the key-loss warning; the keyholder does not hold the new scope until `farcast storage unseal` runs, so a freshly deployed application reports `ErrStorageSealed` until it does. Walk that order deliberately — it is the one sequencing an operator will meet on every first deploy.
