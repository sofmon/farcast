...
**5. Per-application scopes, minted rather than derived, one per application.** `farcast run` mints a scope per application at first deploy — `app/<namespace>/<name>/` — records it in `keys.yaml`, and the bundle carries it. This is the shape 3.2 shipped for the shared `app` scope and the shape [ADR 0008](0008-in-cluster-key-delivery.md) decision 3 said derived scopes can later stand beside: minting reaches the same place (no master in the cluster, compromise bounded per scope, KEK rotatable) and adds nothing at rest, so the derivation freeze stays untouched. An application's secrets subtree moves inside its scope — `app/<ns>/<name>/secrets/<name>` — which changes only what Planck renders into `FARCAST_SECRETS_PREFIX`, because [ADR 0017](0017-application-secrets.md) decision 5 made the prefix something the platform gives and nothing derives.

The shared `app` scope remains, holding what is already there. **Migration is a copy the operator runs, not an automatic rewrite.** A background sweep that moved objects between key spaces is exactly the mechanism that stranded a file in [ADR 0008](0008-in-cluster-key-delivery.md) decision 9, and the honest answer is `storage cp` under the operator's eye, with the old scope retired when it is empty.
...
- **Identity on the data path, per-application leaves, per-application scopes** — the first storage-touching work after Phase 5, and it can land alone: it closes [ADR 0017](0017-application-secrets.md)'s gap whether or not a thin device ever exists. Sequenced by the operator; this ADR does not pull it into a phase that has not been planned.
...
2. **Derived scopes frozen** ([ADR 0008](0008-in-cluster-key-delivery.md) decision 3's 4.x commitment, still open) — decision 5's minted scopes should then be re-examined, not replaced.
