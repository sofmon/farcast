...
**2. The boundary is the instance, not the application — and this is written where a reader will hit it.** *(Superseded 2026-09-11 by [ADR 0018](0018-thin-device-storage.md) decisions 1 and 5; see the note at the end of this section.)* A secret is confidential from the cloud provider, from anyone with the bucket, and from anything outside the instance. It is **not** confidential from another application inside the same instance. That sentence appears in the SDK's `SecretsAPI` documentation, in the `farcast secret` help text, in the ConfigMap Planck renders, and here. A capability called Secrets that quietly promised less than its name is worse than no capability, because an operator would put a payment credential behind it.
...
*Amended 2026-09-10, after [the 5.3 walk](../runbooks/phase-5-3-validation.md).* This decision leaned on accessors that did not work in production: neither `FARCAST_APP_NAME` nor `FARCAST_INSTANCE_ID` had ever been set in a deployed pod, so `AppName()` answered with the executable's base name and `InstanceID()` with `"local"`. The refusal was sound and its replacement was not wired — worth recording, because "the alternative already exists" is the sentence that made the refusal reasonable, and nobody had checked it against a running application.
...
*Settled 2026-09-10 by [ADR 0018](0018-thin-device-storage.md).* Identity on the data path is its decision 1, per-application scopes its decision 5, and the application leaf's credential class its decision 6.

*Decisions 1 and 5 implemented 2026-09-11, and decisions 2, 3 and 4 above are superseded.* The data path authenticates every caller, and every application has its own scope: `app/<namespace>/<name>/`, minted when it is deployed, with its secrets inside it. An application's keys cannot compute the stored name of a neighbour's object, let alone open one, and its leaf does not reach the neighbour's scope to ask — **ordinary objects as well as secrets**. The boundary this ADR called the instance is now the application.

What survives unchanged is decision 3's rule about the OPERATION rather than the owner: no application creates or destroys a secret, whoever it is. Provisioning stays an operator act, and the operator (or a thin device) does it through the keyholder.

The shared `app` scope this ADR was written against no longer exists. Read the rest of this document as the record of why the boundary was where it was, not as a description of where it is.
