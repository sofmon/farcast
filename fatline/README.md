...
fatline/
├── README.md                       ← this file
├── Containerfile                   ← REFERENCE build of the data-plane image (distroless, non-root, digest-pinned bases); the CLI builds the deployed image itself
...
- **`fatline/identity`** and **`fatline/deploy`** (public, added 2.3) are the operator-side bootstrap surface `farcast connect` consumes: `identity` mints the per-instance CA + operator/server leaves and assembles the dial credential (wrapping `internal/crypto` so it stays internal); `deploy` renders FatLine's own Autopilot-compliant workload YAML — the same render [`farcast redeploy`](../farsight/cli/README.md) re-applies to an already-connected instance, so a replacement workload can never be shaped differently from the one the bootstrap laid down. That rendered pod template carries a `farcast.sofmon.com/mtls-hash` annotation fingerprinting the mounted mTLS material: Kubernetes updates a mounted `Secret` in place and FatLine reads its certificate once at start-up, so without the fingerprint a rotation would leave the Deployment spec byte-identical — restarting nothing, while `kubectl rollout status` reports success against a Pod still serving the old certificate. Neither depends on a Kubernetes client ([ADR 0006](../docs/adr/0006-connect-bootstrap-kubectl.md)).
...
**Where the deployed image comes from.** `deploy` renders the workload; it does not build the image — and neither does an operator with a container engine. The [FarSight CLI](../farsight/cli/README.md) builds it ([ADR 0007](../docs/adr/0007-instance-owned-image-registry.md)): it compiles `./fatline/cmd/fatline` for `linux/amd64` with the local Go toolchain (`CGO_ENABLED=0`, `-mod=vendor`, `-trimpath` — hermetic, from vendored source), lays the static binary onto a digest-pinned distroless base, and pushes the result to the instance's **own** registry at `<instance-registry>/system/fatline:<version>` through a standard-library OCI client. No docker, no podman, no daemon, and no Sofmon-published feed in the runtime path. What the CLI then hands `deploy.Config.Image` is the digest (`image@sha256:…`), not the tag, so whoever can later write that tag cannot swap FatLine under a running instance — an explicitly passed `--fatline-image` is deployed exactly as the operator named it. `fatline/Containerfile` reproduces the same image independently — both of its bases pinned by digest — as the **reference** build: for verification, or for an operator who does have an engine.

Two commands walk that path, and they walk *the same code*: `farcast connect` on the first deploy, and `farcast redeploy` for an instance that is already connected — how a fix to the boundary reaches a running instance without destroying it. Image resolution, the build confirmation and the digest pin are shared rather than copied, so the bootstrap and the fix that follows it cannot drift on where the instance's network boundary gets its code from. Intent differs in exactly one place: on a redeploy an explicit `--source` **forces** a rebuild, because the tag tracks the *CLI's* version rather than FatLine's — without the force, a preflight against a connected instance would resolve the image already sitting in its registry and report an unshipped fix as shipped.
...
- **Rotation & revocation.** Rotation is re-issuing a leaf from the held CA and re-applying the workload — the pod template's `mtls-hash` annotation ([above](#architecture--package-layout)) is what makes changed material a *spec* change, so the new leaf reaches the running process instead of landing in an updated `Secret` behind a Pod that never restarted. There is no CRL/OCSP — revocation is short leaf lifetimes plus a FatLine-side allowed-SAN/serial set, correct for a single sovereign instance (with a documented revocation latency — see [Known limitations](#known-limitations)).
...
| 4.1 | [TechnoCore](../technocore/README.md) classifies FatLine last-to-die, so a cost-limit shutdown can never stop the tunnel ([ADR 0009](../docs/adr/0009-technocore-kernel-and-cost-metering.md)) |
...
**Recovery of in-cluster storage runs through the FatLine tunnel, and only half of that floor is now fixed.** [ADR 0008](../docs/adr/0008-in-cluster-key-delivery.md) makes the operator's unseal push travel over this tunnel. FatLine now runs **two replicas behind a PodDisruptionBudget** ([ADR 0009](../docs/adr/0009-technocore-kernel-and-cost-metering.md) decision 11), matching `datasphered`, so the node-upgrade window that seals storage no longer drains the tunnel with it. Two things remain open. The budget constrains *voluntary* disruption only — a full node-pool walk or a zonal loss still takes both replicas, exactly as [ADR 0008](../docs/adr/0008-in-cluster-key-delivery.md) decision 6 records for storage. And the **last-to-die classification is not yet implemented**: until TechnoCore honours it (4.1), a cost-limit shutdown can still stop FatLine and leave storage unsealable while the instance keeps billing. The keeper fleet the same ADR plans makes the replica count a prerequisite rather than an improvement: no keeper, however many devices are enrolled, can re-seed through a drained tunnel.
