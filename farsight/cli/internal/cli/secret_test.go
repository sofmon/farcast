...
func TestSecretSetStoresUnderTheApplicationsOwnPrefix(t *testing.T) {
	...
	if err := setSecret(t, env, []string{"prod", "api", "DB_PASSWORD"}, &secretSetCommand{}); err != nil {
...
// different packages from different inputs, and a divergence stores a secret
// nothing will ever fetch.
func TestSecretKeyMatchesWhatPlanckHandsTheApplication(t *testing.T) {
	root := datasphere.DefaultScopePrefix + datasphere.SecretsSegment + "/"

	rendered, err := translate.Render(translate.Config{
		Manifest: parser.Manifest{
			Name: "my-platform",
			Apps: []parser.App{{Name: "api", Containerfile: "./Containerfile"}},
		},
		Images:            map[string]string{"api": "reg/api@sha256:" + strings.Repeat("a", 64)},
		Credentials:       map[string]string{"api": "cred"},
		StorageScope:      datasphere.DefaultScopeName,
	...
	key, err := secretKey(root, "api", "DB_PASSWORD")
	if err != nil {
...
func TestSecretSetTrimsTheShellsNewline(t *testing.T) {
	env, out, _, _ := secretEnv(t, output.ModeHuman, "hunter2\n")
	if err := setSecret(t, env, []string{"prod", "api", "TOKEN"}, &secretSetCommand{}); err != nil {
		t.Fatalf("secret set: %v", err)
	}
	if got := readStored(t, dataSession(t, env), "app/secrets/api/TOKEN"); got != "hunter2" {
	...
	if err := setSecret(t, env, []string{"prod", "api", "TOKEN"}, &secretSetCommand{raw: true}); err != nil {
...
func TestSecretSetRefusals(t *testing.T) {
	...
	t.Run("a name that would leave the subtree", func(t *testing.T) {
		for _, name := range []string{"../../master/key", "a/b", ".hidden", ""} {
			...
			if err := setSecret(t, env, []string{"prod", "api", name}, &secretSetCommand{}); err == nil {
	...
	t.Run("an application name no manifest could carry", func(t *testing.T) {
		for _, app := range []string{"API", "1api", "api/x", ""} {
			env, _, _, _ := secretEnv(t, output.ModeHuman, "v")
	...
	t.Run("replacing without --force", func(t *testing.T) {
		...
		if got := readStored(t, dataSession(t, env), "app/secrets/api/TOKEN"); got != "first" {
			t.Errorf("the refused write changed the value to %q", got)
		}

		env.In = strings.NewReader("second")
		if err := setSecret(t, env, []string{"prod", "api", "TOKEN"}, &secretSetCommand{force: true}); err != nil {
			t.Fatalf("secret set --force: %v", err)
		}
		if got := readStored(t, dataSession(t, env), "app/secrets/api/TOKEN"); got != "second" {
...
func TestSecretLsNamesOnly(t *testing.T) {
	...
	if err := setSecret(t, env, []string{"prod", "web", "SESSION_KEY"}, &secretSetCommand{}); err != nil {
		t.Fatalf("secret set: %v", err)
	}
	out.Reset()
	...
	out.Reset()
	if err := (&secretLsCommand{}).Run(context.Background(), env, []string{"prod", "api"}); err != nil {
...
func TestSecretRm(t *testing.T) {
	...
	err := (&secretRmCommand{}).Run(context.Background(), env, []string{"prod", "api", "TOKEN"})
	if err == nil {
	...
	if err := (&secretRmCommand{assumeYes: true}).Run(context.Background(), env, []string{"prod", "api", "TOKEN"}); err != nil {
		t.Fatalf("secret rm -y: %v", err)
	}
	if keys := storedKeys(t, dataSession(t, env), "app/secrets/"); len(keys) != 0 {
	...
	err = (&secretRmCommand{assumeYes: true}).Run(context.Background(), env, []string{"prod", "api", "TOKEN"})
	if err == nil || !strings.Contains(err.Error(), "no secret named") {
		t.Errorf("err = %v, want a report that there is no such secret", err)
	}
}

// Applications are handed the RECORDED prefix. If the keyring's scope owns a
// different one, a secret written here is one nothing can read — and there is
// no way to tell from here which of the two is the mistake.
func TestSecretsRootRefusesADivergentPrefix(t *testing.T) {
	env, _, _, dir := secretEnv(t, output.ModeHuman, "v")
	meta, err := dir.LoadInstanceMetadata("prod")
	if err != nil {
		t.Fatalf("LoadInstanceMetadata: %v", err)
	}
	meta.Keyholder.ScopePrefix = "somewhere-else/"
	if err := dir.SaveInstanceMetadata("prod", meta); err != nil {
		t.Fatalf("SaveInstanceMetadata: %v", err)
	}
	err = setSecret(t, env, []string{"prod", "api", "TOKEN"}, &secretSetCommand{})
	if err == nil || !strings.Contains(err.Error(), "somewhere-else/") {
		t.Errorf("err = %v, want a refusal naming both prefixes", err)
	}
}
