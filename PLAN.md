...
- **Planned ADR — thin-device storage through the keyholder: not written.** It is the piece that would also close [ADR 0017](docs/adr/0017-application-secrets.md)'s isolation gap, since both turn on what the keyholder serves an operator leaf versus an application one
