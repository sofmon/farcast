...
Phases 1–4 and Phase 5's first three sections are **validated live against real cloud infrastructure**, not only unit-tested — walked against a [runbook](docs/runbooks/), with every defect each walk found recorded in it. Five things are deliberately not claimed: section 3.2's own runbook is only partly walked (its keyholder is exercised by every Phase 4 walk, but the deliberate-seal and node-upgrade paths are not); the two defects [the 5.2 walk](docs/runbooks/phase-5-2-validation.md) found are fixed but their fixes are not themselves walked; **section 5.4 is unwalked**, and its [runbook](docs/runbooks/phase-5-4-validation.md) needs a second machine to mean anything; the per-application storage identity and key spaces of [ADR 0018](docs/adr/0018-thin-device-storage.md) are built and unwalked, with five re-walk criteria waiting in [the 5.3 runbook](docs/runbooks/phase-5-3-validation.md); and no cost figure has yet been reconciled against a real invoice. See [PLAN.md](PLAN.md) for what each section contains and where it stands.
...
### DataSphere
DataSphere is the storage abstraction layer. It proxies all file storage and retrieval, hiding the underlying cloud provider (S3, GCS, or any object store) behind a uniform interface. Before any data leaves the instance, DataSphere encrypts it — the cloud provider only ever sees encrypted blobs. Combined with FatLine's encryption in transit, the cloud provider never sees a plaintext byte or a logical name. It is blind to *content*, not to everything: object counts, individual object sizes, the shape of your key namespace, and access timing all remain visible. [What the cloud still sees](datasphere/README.md#what-the-cloud-still-sees) states exactly what is and is not hidden.
...
> Phases 0 through 4 are complete and Phase 5 is under way. The lifecycle works end to end and is **validated against real cloud infrastructure**, not only unit tests.
>
...
>
> Section 5.3 completes the SDK's environment: `farcast.Config()` reads an application's configuration and **refuses the platform's own namespace** — one entry of which is the application's egress credential — and `farcast.Secrets()` reads secrets that live as encrypted objects in the instance's storage, never as a Kubernetes Secret. The boundary that buys was stated plainly rather than implied — at the time, a secret was confidential from the cloud and **not** from a neighbouring application, because every application shared one storage scope. [ADR 0018](docs/adr/0018-thin-device-storage.md) has since closed that: every application has its own scope and its own identity, so the boundary is the application. What the keyholder enforces on top is unchanged — no application creates or destroys a secret.
>
> Section 5.4 puts the first keeper on the operator's own hardware: a device holding a derived bundle and its own leaf — never the keyring, never the CA key — that hands a restarted keyholder its material back without waking anyone. It refuses a deliberate seal, refuses beyond a budget, and writes every push to a local ledger the cloud can neither reach nor erase. Because that budget is a tripwire a patient adversary stays under, the keyholder now labels each of its processes so `keeper status` can ask the sharper question: not how often we re-seeded, but **how many distinct processes** we re-seeded. Two pushes into one live process is what a solicited handover looks like from outside. It is delivered for the desktop and **unwalked**, and two things it does not do are stated rather than implied — a desktop cannot bind the material to hardware, and the in-cluster key-id pin that would make revocation bind a modified device is specified and unbuilt. The section's planned ADR — [thin-device storage through the keyholder](docs/adr/0018-thin-device-storage.md) — found that a tablet reading storage and two applications being unable to read each other's are the *same* missing thing, identity on the keyholder's data path — and that prerequisite is now built, together with the per-application scopes on top of it: the keyholder admits only callers presenting a leaf from the instance CA and refuses them at the handshake rather than in a handler, and every application has its own key space — so a neighbour cannot compute the stored name of its objects, let alone open one. `farcast run` mints each scope and hands it to a serving keyholder before the workloads exist, and **will not unseal a sealed one**, because pushing a bundle to a sealed keyholder *is* an unseal and a deploy must not perform that as a side effect. The shared application scope is gone. This work sits beyond Phase 5's own sections, is not counted in them, and is **unwalked**.
>
...
| Planck | 🟡 In progress | 🟡 GKE Autopilot provider, instance image registry, manifest→workload translator (each application gets its own storage identity and key space), in-instance build and manifest read |
| FatLine | 🟡 In progress | 🟡 Core proxy: mTLS tunnel, deny-by-default egress, per-application egress policy |
| DataSphere | 🟡 In progress | 🟡 Encrypting store, blob formats v1+v2, keyring, GCS adapter, in-cluster keyholder with a mutually-authenticated data path, one key space per application, operator-only secrets subtree |
...
| SDK | 🟡 In progress | 🟡 Go: logging, storage, config and secrets live — a `Secret` that refuses to print or marshal itself, and a storage client that proves which application it is; AI is Phase 6 |
