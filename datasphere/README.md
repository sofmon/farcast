...
Stated here because overpromising privacy is worse than not promising it. With everything above in place, the storage provider still observes: the bucket's existence and name; object **count**; individual ciphertext **sizes** (≈ plaintext + 131 B + sealed name); **logical name length, quantized to 32-byte buckets** (the sealed-name length is a plaintext header field and is visible again as the metadata mirror's size); creation and access **timestamps and patterns**; the tree **shape** — depth via token count, and shared logical path prefixes visible as shared stored prefixes, so directory clustering (including the partition the reserved `system/` and `app/` first segments create) is legible even though the names are not — and everything it can infer from those. Chaining means equal segment names under *different* parents do **not** correlate. Four more, added after the 3.2 live runs. A stored name is a deterministic function of the logical key, so an object keeps the same stored name for its whole life and **every overwrite is an independent exact observation of its size** — a frequently rewritten object leaks a size *series*, not a single size, and a series is strictly more informative than one sample. Ranged reads expose byte offsets within an object, and a resumable upload exposes its windows. The version byte at offset 4 records which API wrote the object — `Write` or `WriteStream` — as a plaintext per-object flag. And the request principal separates an operator's tunnelled access from in-cluster access in the provider's own logs.

Hidden: name content, exact name length within its 32-byte bucket, and every byte of data. Not defended: rollback and suppression (above). **Padding is deliberately rejected for now**, and on efficacy rather than on the cost pillar. A small bounded pad is cheap — a mean kilobyte across a million objects is a couple of cents a month — so this spec will not pretend cost is the obstacle. The obstacles are three. First, an additive pad *redrawn per write* is defeated by the deterministic stored name: the provider accumulates a sample per overwrite and recovers the true size from their minimum, with residual error falling as R/(N+1) — roughly 20 bytes after a hundred rewrites of a 2 KiB pad, and it decays fastest on the small mutable objects most worth hiding. Second, a reader-discarded pad of writer-chosen content is the smuggling channel `unpadName` already refuses for names; permitting it here at kilobyte scale would be incoherent. Third, a pad lives under the DEK inside the authenticated plaintext, so unlike the wrap fields it has no header-rewrite migration path — changing the scheme later costs a full decrypt-and-re-seal of every object. A *deterministic* scheme (bucketing, or Padmé) has neither of the first two problems and remains open, but its parameter cannot be chosen without a measured size distribution, and no application can be deployed until 4.3. `farcast storage usage` reports that distribution now, so the decision is made on data when it is made. The class where size *is* content — a value drawn from a small set of differing lengths — is better fixed above this layer, by padding to a fixed slot at the point that knows the data's shape; 5.3's secrets are the first such caller. The version byte is the door; adopting padding or access-pattern defenses is a threat-model ADR, not a patch.
...
**Since [ADR 0018](../docs/adr/0018-thin-device-storage.md) decision 1 the data path is mutually authenticated, and the network is one of two controls rather than the only one.** Until then [`sdk/go/storageclient.go`](../sdk/go/storageclient.go) presented no client certificate: an application verified that it was talking to *the* keyholder, the keyholder could not tell one application from another, and the NetworkPolicy was the whole of access control. Now every application presents a leaf the instance CA issued at `farcast run`, naming it `farcast://<instance>/app/<namespace>/<name>`; the keyholder derives what the caller may reach from that name and admits nothing it cannot identify — at the handshake, not in a handler. The policy stays: on a provider whose network enforces less, the identity survives.

**What a role is served.** The operator and a thin `device` leaf reach every scope the keyholder holds. An `app` leaf reaches exactly one: its own, `app/<namespace>/<name>/`, named by the namespace and application on the leaf. A `keeper` leaf is refused at the handshake; a keeper leaf never reads, exactly as a data leaf never pushes. The scope header the SDK sends remains as a cross-check refused on mismatch, and is no longer what authorizes.

**One scope per application, which is what makes the separation cryptographic.** Every application's scope carries its own name key and KEK, minted by `farcast run` when the application is deployed and recorded in `keys.yaml` before anything is. A neighbour's keys cannot compute the stored name of an object outside their own subtree, let alone unwrap it — so the boundary does not depend on the keyholder enforcing a rule, and holds for ordinary objects as well as secrets. There is deliberately **no scope owning `app/` itself**: a scope owns a whole subtree, so one owning the root and one owning `app/<ns>/<name>/` would both claim the same keys under two different name keys, and `AddScope` refuses that pairing.

A keyring therefore starts with no scopes, and a bundle may carry none. That is an instance with no applications, not an incomplete one — and its keyholder still unseals, so it is ready for the first application that arrives. A *wiped* bundle is a different thing and is still refused.
...
### The secrets subtree: no application creates or destroys a secret (Phase 5.3, narrowed by ADR 0018)

One key-space rule reaches into DataSphere itself: a key under `<scope>/secrets/` may not be written or deleted by an application. `PUT` and `DELETE` from an `app` leaf are refused with `keyholder.ErrSecretsReadOnly`, reported as the frozen `permission` code, before any request body is read. The operator and a thin device provision secrets through the same path, which is what lets `farcast secret set` run from a machine that holds no keyring.

The rule is about the **operation**, not about whose secret it is. When it was written the data path could not tell one application from another, so "not whose, but that no application writes one" was the only honest rule available. Since [ADR 0018](../docs/adr/0018-thin-device-storage.md) an application's secrets are inside its own scope, so *whose* is answered before the key is decoded — a neighbour's leaf does not reach the scope at all. A second check keyed on the path would be a second place for the same rule to live and drift, and was removed with the layout that needed it.

Listing an application's **own** secrets subtree is not refused; listing anything in another application's scope is, by the scope check rather than by a rule about secrets. A test asserts each.
