...
**What a role is served.** The operator and a thin `device` leaf reach every scope the keyholder holds. An `app` leaf reaches the application scope, and — because the secrets layout already names its owner — its own `secrets/` subtree and nobody else's: reading a neighbour's secret, or listing inside a neighbour's subtree, is refused with the frozen `permission` code. A `keeper` leaf is refused at the handshake; a keeper leaf never reads, exactly as a data leaf never pushes. The scope header the SDK sends remains as a cross-check refused on mismatch, and is no longer what authorizes. Per-application *scopes* — cryptographic separation rather than authorization — are ADR 0018 decision 5 and are not yet built; today every application is entitled to the one shared `app` scope, and the boundary between neighbours is enforced by the keyholder's authorization on the secrets subtree only.
...
### The secrets subtree: applications read their own, and write none (Phase 5.3, narrowed by ADR 0018)

One key-space rule reaches into DataSphere itself: a key under `<scope>/secrets/` may be **read** by the application it belongs to and may not be written or deleted by any application. `PUT` and `DELETE` from an `app` leaf are refused with `keyholder.ErrSecretsReadOnly`, and a read or a listing inside *another* application's subtree with `keyholder.ErrNotAuthorized` — both reported as the frozen `permission` code, before any request body is read. The operator and a thin device provision secrets through the same path, which is what lets `farcast secret set` run from a machine that holds no keyring.

When the rule was written the data path could not tell whose secret a key was — it authenticated the server only — and the honest rule was the narrower one: not *whose*, but that no application writes one. [ADR 0018](../docs/adr/0018-thin-device-storage.md) decision 1 put identity on the path, and the secrets layout is the one subtree whose second segment already names its owner, so it is the one place a per-application boundary is enforceable before every application has its own scope. The match is on the application's *name*, because that is how `farcast secret set` files them.

Listing the secrets **root** is still not refused: the parent prefix is listable, so a refusal there would imply an enumeration boundary that does not exist. Listing **inside** a neighbour's subtree is, because that boundary now does. A test asserts each.
