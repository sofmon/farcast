...
**The 3.2 boundary, fixed now.** The next phase gives in-cluster applications storage, and the path of least resistance there is a Kubernetes Secret — which is cloud-resident storage (etcd on the provider's machines). **No entry of the keyring ever rests on cloud infrastructure.** A KEK in a Secret converts encryption-at-rest into encryption-at-rest-except-the-key and guts this module's reason to exist. FatLine's server leaf in a Secret ([`fatline/README.md`](../fatline/README.md)) is not precedent: that is a rotatable *transport* key whose compromise exposes one listener's future sessions; this key *is the data*. The delivery mechanism is a 3.2-era ADR — candidates: (i) an in-cluster DataSphere service holding the keyring in memory only, delivered over the FatLine tunnel at `farcast connect` (strongest guarantee; storage availability follows the operator's presence after a restart), (ii) per-app HKDF subkeys delivered the same way (bounded blast radius, no master in the cluster), (iii) key-in-Secret, named only to be rejected against the invariant above. The keyring type is memory-constructable precisely so all of (i)/(ii) remain open. **3.2 cannot complete before that ADR resolves** — recorded here so 3.2 doesn't discover it.
